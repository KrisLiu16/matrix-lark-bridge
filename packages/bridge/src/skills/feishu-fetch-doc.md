...
description: |
  获取飞书云文档内容。返回文档的 Markdown 内容，支持处理文档中的图片、文件和画板（需配合 lark_doc_media 工具下载）。
---
...
**文档中的图片、文件、画板需要通过 `lark_doc_media`（action: download）工具单独获取！**
...
1. **先调用 `lark_wiki_node`（action: get）解析 wiki token**：
   ```json
...
| obj_type | 工具 | 传参 |
|----------|------|------|
| `docx` | `lark_fetch_doc` | doc_id = obj_token |
| `sheet` | `lark_sheet` | spreadsheet_token = obj_token |
| `bitable` | `lark_bitable_*` 系列 | app_token = obj_token |
| 其他 | 告知用户暂不支持该类型 | — |
...
1. 调用 `lark_wiki_node`（action: get, token: ABC123）
2. 返回 `obj_type: "docx"`, `obj_token: "doxcnXYZ789"`
3. 调用 `lark_fetch_doc`（doc_id: doxcnXYZ789）

## 工具组合

| 需求 | 工具 |
|------|------|
| 获取文档文本 | `lark_fetch_doc` |
| 下载图片/文件/画板 | `lark_doc_media`（action: download） |
| 解析 wiki token 类型 | `lark_wiki_node`（action: get） |
| 读写电子表格 | `lark_sheet` |
| 操作多维表格 | `lark_bitable_*` 系列 |
