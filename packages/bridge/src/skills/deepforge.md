...
根据确认的设定，生成 deepforge.json。**必须包含飞书 Bot 凭据**（从当前 bridge 配置中读取 app_id/app_secret），这样 Leader 每轮结束后会自动发飞书消息汇报进展：
...
- `feishuAppId` / `feishuAppSecret`: 当前 bridge 的 app_id/app_secret（从环境变量 `LARK_APP_ID`/`LARK_APP_SECRET` 获取）
