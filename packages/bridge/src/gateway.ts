...
function writeSkillsToCLAUDEMd(workDir: string): void {
  try {
    ...
    // Only write a summary index — CC reads full skill files on demand
    ...
    for (const file of files) {
      ...
      summaryLines.push(`- **${name}**: ${desc}  \n  Full guide: \`${fullPath}\``);
    }

    const skillsBlock = [
      MLB_START,
      '# Feishu/Lark Skills Reference',
      '',
      '> Auto-generated by Matrix Lark Bridge. Read the full guide file when you need detailed usage.',
      '',
      ...summaryLines,
      '',
      '## DeepForge 启动规则（强制）',
      '',
      '启动 DeepForge 前**必须**用 AskUserQuestion 与用户反复确认需求，直到用户明确说"确认"/"启动"才可执行。',
      '不要跳过确认步骤，不要自行假设需求。详见 deepforge skill 全文。',
      MLB_END,
    ].join('\n');
...
export class Gateway {
  ...
  private async ensureSession(): Promise<void> {
    ...
    try {
      ...
      // Make deepforge available to CC via env var
      const thisDir = dirname(fileURLToPath(import.meta.url));
      const dfEntry = join(thisDir, '../../deepforge/dist/index.js');
      if (existsSync(dfEntry)) {
        claudeEnv.DEEPFORGE_ENTRY = dfEntry;
      }
