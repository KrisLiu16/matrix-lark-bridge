...
- **本地图片或文件**（如用户在聊天中发送的图片/文件） → 先用 create-doc / update-doc 创建或更新文档文本内容，再用 `lark_api (drive media)` 工具将本地图片或文件追加到文档末尾。如需媒体出现在文档中间特定位置，可先用 create-doc 写好之前的内容，调用 `lark_api (drive media)` 追加图片/文件，最后用 update-doc 的 **append** 模式追加后续内容
