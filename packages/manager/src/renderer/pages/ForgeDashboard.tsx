    ...
    <div className="h-full flex flex-col animate-fade-in">
      ...
      <div className="flex items-center justify-between px-6 py-3 shrink-0
        ...
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-sm font-semibold">Forge Dashboard</h1>
        ...
        {loading && projects.length === 0 ? (
          ...
          <div className="flex items-center justify-center h-full">
            <div className="text-center text-slate-400 dark:text-slate-600">
              <p className="text-sm">Failed to load Forge projects</p>
          ...
          <div className="flex items-center justify-center h-full">
            <div className="text-center text-slate-400 dark:text-slate-600">
              ...
              <p className="text-sm">No Forge projects found</p>
              <p className="text-xs mt-1">Projects in ~/.forge/projects/ and ~/.deepforge/projects/ will appear here</p>
            </div>
          </div>
        ) : selectedProject ? (
          <ProjectDetail
...
function ProjectCard({ project, onClick }: { project: ForgeProject; onClick: () => void }) {
  ...
  const runningColor = project.isRunning
    ? PHASE_COLORS.running
    : PHASE_COLORS[project.phase] || DEFAULT_PHASE_COLOR;
    ...
    >
      ...
      <div className="flex items-center gap-4 text-[11px] text-slate-500 dark:text-slate-400 mb-3">
        <span className="flex items-center gap-1">
          ...
          Iter {project.currentIteration}/{project.totalIterations || '?'}
        </span>
        <span className="flex items-center gap-1">
          <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor">
  ...
  if (!project) {
    return (
      <div className="p-6">
        ...
        <p className="text-sm text-slate-400 mt-4">Project not found</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 animate-fade-in">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        ...
        <StatCard label="Iteration" value={`${project.currentIteration}/${project.totalIterations || '?'}`} />
        <StatCard label="Cost" value={`$${project.totalCostUsd.toFixed(2)}`} />
        <StatCard label="Source" value={project.source} />
      </div>

      {/* Tasks section */}
      {project.tasks.length > 0 && (
        <div className="mb-6">
          <h2 className="text-sm font-semibold mb-3 text-slate-700 dark:text-slate-300">Current Tasks</h2>
          <div className="space-y-2">
            {project.tasks.map((task, i) => (
              ...
              >
                ...
                <span className="text-sm font-medium text-slate-700 dark:text-slate-300 flex-1">{task.role}</span>
      ...
      {iterations.length > 0 && (
        <div className="mb-6">
          <h2 className="text-sm font-semibold mb-3 text-slate-700 dark:text-slate-300">Iterations</h2>
          <div className="space-y-2">
              ...
              >
                <div className="flex items-center justify-between">
                  ...
                  {iter.costUsd !== undefined && (
                    <span className="text-xs text-slate-400">${Number(iter.costUsd).toFixed(2)}</span>
                  )}
                </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-sm font-semibold text-slate-700 dark:text-slate-300">Logs</h2>
