...
- ⚠️ **默认表的空行坑**：`app.create` 自带的默认表中会有空记录（空行）！插入数据前建议先调用 `lark_bitable_record.list` + `batch_delete` 删除空行，避免数据污染
- ✅ **写记录前**：先调用 `lark_bitable_field.list` 获取字段 type/ui_type
...
|---------|------|--------|---------|---------|
| 查表有哪些字段 | lark_bitable_field | list | app_token, table_id | - |
| 查记录 | lark_bitable_record | list | app_token, table_id | filter, sort, field_names |
| 新增一行 | lark_bitable_record | create | app_token, table_id, fields | - |
| 批量导入 | lark_bitable_record | batch_create | app_token, table_id, records (≤500) | - |
| 更新一行 | lark_bitable_record | update | app_token, table_id, record_id, fields | - |
| 批量更新 | lark_bitable_record | batch_update | app_token, table_id, records (≤500) | - |
| 创建多维表格 | lark_bitable_app | create | name | folder_token |
| 创建数据表 | lark_bitable_table | create | app_token, name | fields |
| 创建字段 | lark_bitable_field | create | app_token, table_id, field_name, type | property |
| 创建视图 | lark_bitable_view | create | app_token, table_id, view_name, view_type | - |
...
- **使用场景完整示例** — 见下方"核心使用场景"章节

**何时查阅**:
- 创建/更新字段时收到 `125408X` 错误码（property 结构错误）→ 检查字段 Property 配置
- 写入记录时收到 `125406X` 错误码（字段值转换失败）→ 检查记录值数据结构
- 需要完整的操作流程和参数示例 → 参考下方"核心使用场景"章节
...
**强制流程**：
1. 先调用 `lark_bitable_field.list` 获取字段的 `type` 和 `ui_type`
2. 根据上表构造正确格式
...
> **完整示例**: 以下场景覆盖创建表模式对比、空行处理、附件上传、关联字段等
